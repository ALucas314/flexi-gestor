                  ...
                  filteredSuppliers.map((s, index) => (
                    <TableRow key={s.id} className="border-neutral-100 hover:bg-neutral-50 transition-colors duration-150">
                      ...
                      <TableCell className="px-4 py-3">
                        {s.rua ? `${s.rua}${s.numero ? `, ${s.numero}` : ''}${s.complemento ? ` - ${s.complemento}` : ''}` : '—'}
                        {s.bairro ? <span className="block text-xs text-muted-foreground">{s.bairro}</span> : null}
                      </TableCell>
                      <TableCell className="px-4 py-3">
                        {s.cidade || s.estado ? `${s.cidade || ''}${s.estado ? `/${s.estado}` : ''}` : '—'}
                      </TableCell>
                      <TableCell className="px-4 py-3">{s.cep || '—'}</TableCell>
